...
if (file_end %in% c("mean")) {  
  ...
  if (SRVY %in% c("BS", "NBS", "EBS")) {
    ...
    fig_cap <- paste0(
      "The Gulf of Alaska bottom trawl survey. ", 
      goaai_regions0)
  }
  ...
  if (nrow(loc) == 0) {
    ...
    temp1 <- ""
    temp11 <- as.character(unique(dat_survreg$vessel_shape)); temp11 <- temp11[!is.na(temp11)] # vessels in data
  ...
  temp00 <- paste0("The near real-time ocean bottom temperatures depicted were collected ", 
                    ifelse(min(as.Date(dat$date), na.rm = TRUE) == max_date, 
                           paste0("on ", format_date_txt(x = min(as.Date(dat$date)+1, na.rm = TRUE), "%B %d %Y")), 
                           paste0(format_date_txt(x = min(as.Date(dat$date)+1, na.rm = TRUE), "%B %d"), 
                                  "-", 
                                  format_date_txt(x = as.Date(max_date), "%B %d"))), ". ")
    temp01 <- paste0("The near real-time ocean bottom temperatures will be depicted on ",
                     format_date_txt(x = min(as.Date(dat$date)+1, na.rm = TRUE), "%B %d %Y"),
                     ". ")
