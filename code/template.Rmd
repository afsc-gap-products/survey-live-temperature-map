...
if (file_end %in% c("mean")) {
    ...
    gsub(pattern = "\n", 
         replacement = paste0(": "), 
         x = stringr::str_to_sentence(plot_title)), 
    ...
    fig_cap <- fig_alt <- paste0(
    gsub(pattern = "\n", 
         replacement = paste0(", comparing bottom temperatures from ", maxyr, " to "), 
         x = stringr::str_to_sentence(plot_title)), 
    ". Credit: NOAA Fisheries. ")
...
#     paste0(" and ",text_list(x = paste0(unique(survey_area$bathymetry$METERS), "m"))," bathymetry. ")))
    ...
    for (iii in 1:length(unique(dat_max_date$vessel_shape))) {
      ...
      temp <- c(temp, 
                paste0(# nrow(temp1), 
                       ...
                       text_list(x = 
                                   paste0(temp1$station,
                                            " (",temp1$lat,"°N, ",temp1$lon,"°W; ",
                                            gsub(pattern = " ", 
                                                 replacement = "", 
                                                 x = temp1$var_bin, fixed = TRUE),"°C)")), 
  ...
  fig_cap <- fig_alt <- paste0(
    ...
    ifelse(data_source == "gd", 
           "Data have NOT been through final review. Final, validated haul and catch data will be publicly accessible on FOSS platform (https://www.fisheries.noaa.gov/foss/f?p=215:200:1899488600966:Mail:NO:::). ", 
           "Data have been through final review. Final, validated haul and catch data collected at each station), are be publicly accessible on FOSS platform (https://www.fisheries.noaa.gov/foss/f?p=215:200:1899488600966:Mail:NO:::). "))
