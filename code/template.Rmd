...
if (file_end %in% c("mean")) {  
  ...
  fig_cap <- paste0(
    ...
    "The ", maxyr,
    " near real-time ocean bottom temperature anomaly in the NOAA Fisheries AFSC Groundfish Assessment Program ",
    text_list(surveys),
    " bottom trawl survey",
    ifelse(length(surveys)>1, "s", ""),
    ". ",
  ...
  # } else if (date_entered[length(date_entered)] == max_date & sum(is.na(dat_plot$var))>0) {
    ...
    temp1 <- ""
    temp11 <- unique(dat_plot$vessel_shape); temp11 <- temp11[!is.na(temp11)] # vessels in data
    temp12 <- unique(dat_max_date$vessel_shape) # vessels from maxdate
    temp13 <- setdiff(temp11, temp12)
    if (length(temp13)>0) {
