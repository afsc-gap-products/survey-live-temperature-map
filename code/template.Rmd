...
surveys <- gsub(pattern = " Eastern", replacement = " eastern", 
                x = unique(dat_plot$region_long))
surveys <- gsub(pattern = " Northern", replacement = " northern", 
                x = unique(dat_plot$region_long))
...
if (SRVY %in% c("AI", "GOA")) {
  goaai_regions <- survey_area$survey.grid %>% 
    st_centroid() %>% 
    st_transform(crs = CRS("+proj=longlat")) %>%
    st_coordinates() %>% 
    data.frame() %>%
    dplyr::mutate(region = unlist(survey_area$survey.grid$region)) %>% 
    dplyr::group_by(region) %>% 
    dplyr::summarise(minlon = min(X, na.rm = TRUE)) %>% 
    dplyr::arrange(minlon)
  
  goaai_regions <- paste0("This survey covers the ", 
                          text_list(goaai_regions$region), 
                          " regions, which are sampled randomly each year. ")
} else {
  goaai_regions <- ""
}

intro0 <- paste0(
  "NOAA Fisheries Alaska Fisheries Science Center's Groundfish Assessment Program conducted the ", 
  text_list(surveys),
  " bottom trawl survey",
...
if (file_end %in% c("mean")) {
  ...
  if (SRVY %in% c("BS", "NBS", "EBS")) {
    fig_cap <- paste0( 
      "The grid of designated stations in the ",text_list(surveys)," bottom trawl survey area",
      ifelse(length(surveys)>1, "s", ""), 
      " as well as the ", 
      text_list(x = paste0(unique(survey_area$bathymetry$METERS), "m")),
  ...
  if (show_planned_stations) {
    ...
    stations1 <- paste0(stations1, "Allocated stations that have not yet been sampled are shown as gray dots. ")
  }
